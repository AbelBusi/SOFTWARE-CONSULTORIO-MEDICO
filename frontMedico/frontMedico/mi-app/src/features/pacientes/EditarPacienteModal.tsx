...
import { type Paciente } from "./DetallePacienteModal";

interface Props {
    paciente: Paciente | null;
    onClose: () => void;
    onSave: (updated: Paciente) => void;
}
...
export default function EditarPacienteModal({ paciente, onClose, onSave }: Props) {
    const [form, setForm] = useState<Paciente | null>(null);
    const [saved, setSaved] = useState(false);

    useEffect(() => {
        if (paciente) {
            setForm({ ...paciente, alergias: [...paciente.alergias] });
            setSaved(false);
        }
    }, [paciente]);

    // No renderizar si no hay paciente o form
    ...
    const setAlergias = (e: React.ChangeEvent<HTMLInputElement>) =>
        setForm((f) => f ? { ...f, alergias: e.target.value.split(",").map((s) => s.trim()).filter(Boolean) } : f);
    ...
    return (
        /* Backdrop editar — z-50, encima del detalle (z-40) */
        <div
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
                    <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <Field label="DNI" icon={IdentificationIcon}>
                                    <input className={inputCls} value={form.dni} onChange={set("dni")} maxLength={8} required />
                                </Field>
                                <Field label="Fecha de nacimiento" icon={IdentificationIcon}>
                                    <input type="date" className={inputCls} value={form.fechaNacimiento} onChange={set("fechaNacimiento")} required />
                                </Field>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <Field label="Sexo" icon={UserIcon}>
                                    <select className={selectCls} value={form.sexo} onChange={set("sexo")}>
                        ...
                        <div className="space-y-3">
                            ...
                            <Field label="Dirección" icon={MapPinIcon}>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <Field label="Seguro" icon={ShieldCheckIcon}>
                                    <select className={selectCls} value={form.seguro} onChange={set("seguro")}>
                                        {["SIS", "EsSalud", "Rimac", "Pacífico", "Mapfre", "Particular"].map((s) => (
                                            <option key={s}>{s}</option>
                                        ))}
                                    </select>
                                </Field>
                                <Field label="Grupo sanguíneo" icon={HeartIcon}>
                                    <select className={selectCls} value={form.grupoSanguineo} onChange={set("grupoSanguineo")}>
                            ...
                            <Field label="Alergias (separar con coma)" icon={HeartIcon}>
                                <input
                                    className={inputCls}
